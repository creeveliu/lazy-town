...
已实现一个面向中文用户的「热门游戏 + 电影放映表」网站，优先保证：
- 极简展示
- Vercel 可部署
...
1. 页面与交互
- 首页包含两张表：`热门游戏发售表`、`电影放映表`
- 游戏字段：`海报 / 发售日 / 游戏名 / 平台 / 热度`
- 电影字段：`海报 / 上映日 / 电影名 / 类型&地区 / 热度`
- 游戏名可点击跳转外部详情页
- 电影名可点击跳转外部详情页
...
2. 数据源与筛选
- 数据源：第三方发售页（PC / PS5 / Switch）
- 电影数据源：豆瓣即将上映页（`movie.douban.com/coming`）
- 时间范围：未来 1 年
- 热度阈值：`> 4000`
- 排序：按发售日期升序
- 跨平台去重：按源链接合并平台字段
...
3. 抓取与存储架构
- 抓取模块：`src/lib/game-source.ts`
- 抓取模块：`src/lib/movie-source.ts`
- 数据库存取：`src/lib/db.ts`
...
- 同步逻辑：抓取 -> 过滤 -> 全量重建 `games` / `movies` 表
- 同步日志：写入 `sync_logs`
...
- 已验证手动同步成功（示例：`syncedCount = 11`, `movieSyncedCount = 27`）

## 当前约束与后续建议

1. 当前约束
- 电影来源当前未抓取封面图，页面显示占位图
