...
import { getOnlineMoviesFromDb } from "@/lib/db";
...
export async function GET() {
  const movies = await getOnlineMoviesFromDb();
  ...
  for (const movie of movies) {
    ...
    const uid = `${movie.url}@online-movies.lazy-town`;
  ...
  return new NextResponse(lines.join("\r\n"), {
    headers: {
      ...
      "Content-Disposition": "attachment; filename=online-movies.ics",
    },
  });
}
